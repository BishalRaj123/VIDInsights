...
# Function to transcribe video using Whisper with timestamps
def transcribe_video(file_path):
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_audio_file:
            ...
            # Transcribe audio (sentence/segment level)
            result = whisper_model.transcribe(temp_audio_file.name, language='en')

            # Format transcript with HH:MM:SS timestamps and sentence text
            transcript = "\n".join(
...
def process_video():
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ''', (session_id, user_id, title, is_youtube, video_path, youtube_id, transcript, summary))
    conn.commit()
    conn.close()
...
def ask_question():
    ...
    cursor.execute("SELECT transcript, user_id FROM session WHERE id = ?", (session_id,))
    session_data = cursor.fetchone()
